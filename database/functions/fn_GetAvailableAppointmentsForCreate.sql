IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[fn_GetAvailableAppointmentsForCreate]') AND type in (N'FN', N'IF', N'TF', N'FS', N'FT'))
	DROP FUNCTION [dbo].[fn_GetAvailableAppointmentsForCreate]
GO
...
-- Description:	Get the available dates and laboratories for a new appointment.
-- =============================================
CREATE FUNCTION [dbo].[fn_GetAvailableAppointmentsForCreate]
(
	@studentId INT	-- The identity of the student who is making the appointment.
)
RETURNS @table TABLE
(
...
BEGIN
	...
	(
		...
		(
			...
			FROM dbo.fn_GetDateTimeRange(SYSDATETIMEOFFSET(), 2) AS R, Laboratories AS L
			INNER JOIN States AS S ON L.FK_State_Id = S.PK_State_Id
			WHERE dbo.fn_StudentHasAppointment(@studentId, R.Date) = 0
			AND dbo.fn_GetLaboratoryAvailableSeats(L.PK_Laboratory_Id, R.Date) > 0
			AND dbo.fn_IsLaboratoryReserved(L.PK_Laboratory_Id, R.Date, R.Date) = 0
			AND dbo.fn_GetStudentWeeklyAppointmentsCount(@studentId, R.Date) < 2
		) InnerDateRange
	) DateRange
	WHERE RN = 1;
	RETURN;
